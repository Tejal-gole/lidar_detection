...
{	ofstream fout;  // Create Object of Ofstream
  ...
  // TODO: Create two new point clouds, one cloud with obstacles and other with segmented plane
	...
	for(int it : inliers->indices)
	{
		...
		//std::cout<<"plane cloud "<< planeCloud<<std::endl;
	}
	...
	// std::cerr << "Model inliers: " << obstCloud->points.size() << std::endl;
	// len =inliers->indices.size();
	// char buffer[1024] = {0}; 
	// valread = read( new_socket , buffer, 1024); 
	// cout<<"length of inliers for socket"<<len<<endl ;

	// send(new_socket ,inliers ,len , 0 );
  	
	for (std::size_t i = 0; i < inliers->indices.size (); ++i)
    	{
    		cout<< typeid(cloud->points[inliers->indices[i]]).name() << std::endl;
    	// std::cerr << inliers->indices[i] << "    " << cloud->points[inliers->indices[i]].x << " "
    //                                            << cloud->points[inliers->indices[i]].y << " "
    //                                            << cloud->points[inliers->indices[i]].z << std::endl;}
	
	// for (std::size_t i = 0; i < obstCloud->points.size (); ++i)
    // 	std::cerr << obstCloud->points[i] << std::endl;
	// fin.open(std::to_string(numb)+"i.txt");
    // fout.open(std::to_string(numb)+"i.txt",ios::app);
    // for (std::size_t i = 0; i < inliers->indices.size (); ++i)
    // {   
    //     if(fin.is_open())
    //     {	fout<<cloud->points[inliers->indices[i]]<< std::endl;
    //         std::cerr <<"tdbshvbhbvuoh"<< inliers->indices[i] <<std::endl;//fout<< segmentCloud.first->points[i] << std::endl;
    //     }
    	}
...
//now can use pi_ptr
	...
	// cout<<"length of inliers for socket"<<len<<endl ;
	// send(new_socket ,inliers ,len , 0 );
	// std::cerr << "Model inliers: " << inliers << std::endl;
  	// for (std::size_t i = 0; i < inliers->indices.size (); ++i)
   //  	std::cerr << inliers->indices[i] << "    " << cloud->points[inliers->indices[i]] << " "<<std::endl;
   //                                             // << cloud->points[inliers->indices[i]].y << " "
   //                                             // << cloud->points[inliers->indices[i]].z << std::endl;
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane_RANSAC(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
		...
		for(int pt_cnt=0;pt_cnt<cloud->points.size();pt_cnt++)
		{
			if(pt_cnt!=idx1||pt_cnt!=idx2||pt_cnt!=idx3)
			{
				...
				if(dis<=distanceThreshold)
				{
					...
					//std::cout<<"point count"<< tempIndices.insert(pt_cnt)<<std::endl;
				}
	...
	for(int index = 0; index < cloud->points.size(); index++)
	{
		...
		{	cloudInliers->points.push_back(point);
			//std::cout<<"inliers points"<< *cloudInliers<<std::endl;
		}
		else
			cloudOutliers->points.push_back(point);
			//std::cout<<"outliers points"<< *cloudOutliers<<std::endl;
	}
	std::cerr << "cloud inliers: " << cloudInliers.size () << std::endl;
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering_euclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
	...
	//std::cout<<"cloud value is :"<< *cloud <<std::endl;
...
{	
    ...
    if (pcl::io::loadPCDFile<PointT> (file, *cloud) == -1) //* load PCD file
...
std::vector<boost::filesystem::path> ProcessPointClouds<PointT>::streamPcd(std::string dataPath)
{
	...
	//for(int i=0;i<(int)paths.size();i++)std::cout<< paths[i]<<std::endl;
    return paths;
